...
def book_ticket(movie_json, user_json, email, tkt_id, no_of_booking, *preferred_seats):
    ...
    # Booking Process
    if number_of_seat_av >= int(no_of_booking):
...
def show_all_the_movies(movie_json):
    f=open(movie_json, "r+")
    content=json.load(f)
    for i in range(len(content)):
        available_seats = content[i]["Total Seating"] - len(content[i]["Booked Seats"])
        print(content[i]["Movie ID"], content[i]["Movie Name"], content[i]["Total Seating"], available_seats, content[i]["Price"], content[i]["Duration"])
    f.close()